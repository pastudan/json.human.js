...
$(function () {
    ...
    //disable context menu for JSON-DOM
    document.getElementById("output").oncontextmenu = function() {return false;};
    ...
    .on("mousedown", "tr",function (e) {
        if( e.button == 2 ) {
            e.stopPropagation();
            e.preventDefault();
            //if right click, toggle display of value
            console.log($(this).find("td:first").toggleClass("collapsed"));
        } else {
            var node = $(this).find("th:first").text();
            nodeList.push(node);
...
//                                    console.log("length:" + nodeList.length, "i: " + i)
                curValue = $hierarchy.val() || "";

                var validIdintifier = validVarRegex.test(nodeList[i]) ? "."+nodeList[i] : "['"+nodeList[i].replace(/'/g, "\\\'")+"']";
                $hierarchy.val(validIdintifier + curValue);
            }
            curValue = $hierarchy.val() || "";
            responseVarName = $("#responseVarName").val();
            $hierarchy.val(responseVarName + curValue);
            $("#value").val(valList[0]);
//                                console.log(nodeList);
    ...
    //clear hierarchy if user's response variable changes
    $("#responseVarName").keyup(function(){
        $("#hierarchy, #value").val("");
    });
